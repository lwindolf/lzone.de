...
export class CheatSheetCatalog {
    ...
    static getInstallable = async (group) => 
        window.Config.groups[group]?.catalog
            ?await fetch(window.Config.groups[group].catalog.source)
    ...
    // Installs a section group (as defined in window.Config.groups)
    static async #installGroup(group) {
        try {
            for (const name of Object.keys(window.Config.groups[group].install)) {
                const repo = window.Config.groups[group].install[name];
                await this.install(group, name, repo, undefined, false);
            }
        } catch (e) {
            console.error(`Error fetching index ${window.Config.indexUrls[group].index}: ${e}`);
        }
    }
    ...
    static async update() {
        ...
        if(!initialRun) {
            ...
            for(const name of Object.keys(window.Config.groups)) {
                if(!sections.nodes[name] && !window.Config.groups[name].removable && name !== "Feeds") {
        ...
        for(const group of Object.keys(window.Config.groups)) {
            if(!window.Config.groups[group].install)
                continue;
